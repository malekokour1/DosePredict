...
############ PK ASSISSTANT SERVER
# NOTES:
# remove scale and BP
# remove timeaboveEff from eff table
# fix pobabilites issue when (for efficacy) when some subjects dont cross the time target.

# take care of units and BP 
# its only PK for this version but PKPD can be added later on
# simulations are in plasma or remove the specification 

# fix the legend colors and add shape: legend colors are related to 90% PI Ribbon
# make the dose in-senstive to charachter, example:   DOSEIN <-  unique(as.numeric(unlist(strsplit(gsub("[^0-9]", "", unlist(input$DOSE)), ""))))
# fix the numerical integration issue
# blood or plasma , maybe you should use CENT rather than CPLASMA
# add specific AUC as from to to in the SAF plots
# fix th PI error
##############################################################################
########################################################Global variables  ####
##############################################################################
...
#deploy:
#library(rsconnect)
#rsconnect::setAccountInfo(name='mokour', token='767BC352FD9734EAD79C923CC9576D1E', secret='43eZJ3wf1Iy0GW/syO+374Ddatl0fO6CpRikKYqY')
...
##############################################################################
##### NOTE: add IV models!!!!!!!!!!!!!!!!!
##### NOTE: Consider other forms of Indirec response models ( induce Kin, inhbit Kin, induce Kout)
####################################################################################################
...
TVIC50=0.02, TVEMAX=.7, TVKG=0.076, Nmax=1000000000000
...
dxdt_PARAS = KG*PARAS*(1-PARAS/Nmax) - INH*PARAS;
...
$CMT GUT CENT PERI1 PERI2 PARAS
$OMEGA @labels ECL EVC EKA EBV EIC EKmax
0.09 0.09 0.09 0.09 0.09 0.09
...
################# SHINY INPUT:
  ...
  SCAIN<-input$SCALE
  BPIN<-input$BP
  #PD: fixed for now, in next version will incude PD models
  IC50<-0.02
  Kmax<-0.7
  KG<-0.076
